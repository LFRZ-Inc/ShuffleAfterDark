...
export default function AccountPage() {
  ...
  const mockProfile: UserProfile = {
    ...
    preferences: {
      favoriteGenres: (userProfile?.preferences as any)?.favoriteGenres || ['romantic', 'couples', 'artistic'],
      blockedTags: (userProfile?.preferences as any)?.blockedTags || ['extreme'],
      preferredDuration: (userProfile?.preferences as any)?.preferredDuration || 'medium',
      contentIntensity: (userProfile?.preferences as any)?.contentIntensity || 'medium',
      shuffleAlgorithm: (userProfile?.preferences as any)?.shuffleAlgorithm || 'smart',
      autoPlay: (userProfile?.preferences as any)?.autoPlay || false,
      showContentWarnings: (userProfile?.preferences as any)?.showContentWarnings || true,
      privateHistory: (userProfile?.preferences as any)?.privateHistory || false,
    },
    stats: {
      totalViews: 1247,
      favoriteCount: 89,
